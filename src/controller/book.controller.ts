...
export const updateBook = async (req: Request, res: Response): Promise<Response<Book>> => {
    ...
    try {
        ...
        const result: ResultSet = await pool.query(QUERY.SELECT_BOOK, [req.params.bookId]);
        if ((result[0] as Array<ResultSet>).length > 0) { // si MySQL no regresa ningún dato, no lo halló
            const result: ResultSet = await pool.query(QUERY.UPDATE_BOOK, [ ...Object.values(book), req.params.bookId]);
            return res.status(Code.OK)
                .send(new HttpResponse(Code.OK, Status.OK, 'Book updated', { ...book, id: req.params.bookId }))
        }
        else {
            return res.status(Code.NOT_FOUND)
                .send(new HttpResponse(Code.NOT_FOUND, Status.NOT_FOUND, 'Book not found'))
...
export const deleteBook = async (req: Request, res: Response): Promise<Response<Book>> => {
    ...
    try {
        ...
        const result: ResultSet = await pool.query(QUERY.SELECT_BOOK, [req.params.bookId]);
        if ((result[0] as Array<ResultSet>).length > 0) { // si MySQL no regresa ningún dato, no lo halló
            const result: ResultSet = await pool.query(QUERY.DELETE_BOOK, [req.params.bookId]);
            return res.status(Code.OK)
                .send(new HttpResponse(Code.OK, Status.OK, 'Book deleted'))
        }
        else {
            return res.status(Code.NOT_FOUND)
                .send(new HttpResponse(Code.NOT_FOUND, Status.NOT_FOUND, 'Book not found'))
